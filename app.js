...
app.get('/blog',(req,res)=>{
    res.status(200).sendFile('blog.html',{root:path.join(__dirname,'views')});
});
app.get('/123',(req,res)=>{
    res.status(200).sendFile('123.html',{root:path.join(__dirname,'views')});
});
app.get('/table',(req,res)=>{
    res.status(200).sendFile('table.html',{root:path.join(__dirname,'views')});
});
//declaring end points for post request
app.post('/form',(req,res)=>{
    var myData = new data(req.body);
    myData.save().then(()=>{
        res.status(200).send("<h1>Your form has been submitted! Thanks</h1>");   //sending response with status code 200 if there is no error
